...
async function handleSlate(req, res, client, body) {
  ...
  const cmds = [
    ...
    ["SADD", "bi:clients", client],
    ["HSET", `bi:client:${client}`, "last", String(t), "lastEvent", mode === "export" ? "export" : "slate-sync", "slateCount", String(items.length)],
  ];
  if (mode === "export") {
    const rec = JSON.stringify({ c: client, ev: "export", n: items.length, t, ip });
