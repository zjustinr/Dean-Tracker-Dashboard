...
import { isSitting } from "../artifacts/dean-dashboard/scripts/lib/tenure.mjs";
...
// The visible switcher list (excludes hidden top100).
...
/**
 * Which indices the header corpus figure counts. EVERY dataset the repository
 * holds, except top100 -- which is a hidden strict subset of r1bschool (98.3% of
 * its records are byte-identical appointments) and would double-count.
 *
 * Deliberately NOT the same list as VISIBLE. VISIBLE gates news-scout's
 * assertDatasetCoverage(), which throws when an id in it has no classifier
 * entry in news-lib.mjs -- so adding an index there to make the corpus figure
 * complete would break the daily appointment scan for every OTHER index. The
 * two lists answer different questions: VISIBLE is "does the news cron cover
 * this?", CORPUS_IDS is "does the repository hold this?". They were one list,
 * and that coupling is why usadminleaders (4,514 records) and uscommunitycollege
 * (1,145) went uncounted.
 */
export const CORPUS_IDS = Object.keys(SPEC).filter((id) => id !== "top100");
...
export function computeCorpus(srcDir) {
  ...
  for (const id of CORPUS_IDS) {
    for (const d of assembleDataset(id, srcDir).deans) {
      ...
      // isSitting(), not "no end year": a handful of historical records carry an
      // end the sources never documented (endLabel "unknown"), and counting those
      // as sitting leaders would put 19th-century founding deans in the header's
      // "leaders in post" figure.
      if (isSitting(d)) sitting++;
      // A roster record (community college) names the incumbent from IPEDS but
      // dates no appointment. It IS a sitting leader and it IS a covered seat,
      // so it counts toward `sitting` and `schools` -- but calling it an
      // appointment would put 1,060 undated, largely uncorroborated names into a
      // number that otherwise means "researched succession event". Counted
      // separately so `appts` keeps its meaning and the split stays inspectable.
      if (d.rosterOnly) roster++;
      if (d.startYear && d.startYear < minYear) minYear = d.startYear;
    }
  }
  // `indices` rides along so the header strip's four figures share one
  // denominator. It used to render DATASET_LIST.length -- the SWITCHER count --
  // beside totals computed from a different set, so the strip quietly described
  // two different corpora at once.
  return { appts: seen.size - roster, sitting, roster, schools: schools.size, indices: CORPUS_IDS.length, from: minYear };
}
