...
- **Signup**: send people to `/?join`. They enter a work email → `POST /api/trial?action=signup` → a one-time link (15 min, `bi:signup:<sha256(code)>`) by Resend. The emailed GET only renders a Continue button; the POST spends the code, so mail-security scanners that prefetch links can't burn it. Verify sets an **HttpOnly** `bi_trial` cookie holding a token with `c = email` and `o = domain`: a rolling 90-day login, re-issued by `/api/trial` once a day while in use.
- **Live entitlement**: for tokens carrying `o`, the token only says who you are. Scope and end date are read from `bi:org:<domain>` on every request (`liveAccess()` in `api/trial.js` and `api/data.js`), so **renewing = saving the org with a new end date**; nobody signs in again. After the end date there is a 14-day grace period (full access, renewal banner via `OrgAccessNotice.tsx`); after that, the free tier with an "access ended" banner. Removing an org ends its members' access.
- **Names**: the page behind the emailed link asks a first-time user for first + last name before spending the code (a blank submit re-shows the form; the link stays good); returning users just see Continue. Stored on `bi:user:<email>` (`firstName`, `lastName`), editable from the Account page (`POST ?action=profile`).
- **Account page** (`AccountDialog.tsx`, header button or `/?account`): profile, plan (org, coverage, live end date / grace, seats), placeholders for saved searches and upgrades, sign out (`POST ?action=signout` — the cookie is HttpOnly, so the server clears it). Data from `GET /api/trial?action=account`.
- **Sign-in email**: table-based HTML with inline styles (what Gmail/Outlook render), the PNG logo at `public/email-logo.png` (SVG doesn't render in mail clients; re-render from `logo.svg` at 144px if the logo changes), brand navy/crimson, plus a plain-text part.
- **Seats** (firm pays per seat): optional `seats` on the org. Members are `bi:org-users:<domain>`; signup refuses a new email once it is full (`no_seats`), verify re-checks after claiming (`?signup=full`), existing members can always sign in again. **Remove** on the usage page (`?unseat=<email>`) frees the seat and ends that person's access live (`liveAccess` state `removed`). Saving an org with blank name/seats keeps the current values; seats 0 = unlimited.
- **Renewal reminders**: a daily Vercel cron (`vercel.json` → `/api/usage`, authenticated by `CRON_SECRET`) emails the owner (`RENEWAL_REMINDER_TO`, else `FEATURE_REQUEST_TO`) at 60/30/7 days before, on the end date, and when grace closes — once per stage per end date (`bi:reminded:*`), with usage numbers. Never emails the firm.
- **Control**: each person is their own client tag, so the existing Block switch works per email. `bi:blocked:@<domain>` blocks the whole org, including tokens already issued (checked in `api/trial.js` and `api/data.js`).
...
Self-serve alongside the $49 Day Pass (both scope `"*"`). Stripe Payment Link (`MONTHLY_PASS_URL` in `FreeTierMeter.tsx`; the card is hidden while it's empty) → `api/stripe-webhook.js` keeps `bi:sub:<email>` in sync: checkout (mode=subscription) creates a provisional 32-day record and emails a 24-hour sign-in link (via `issueSignInLink`, exported from `api/trial.js`); `invoice.paid` sets `until` to the paid period's end; `invoice.payment_failed` → `past_due` (14-day grace); `customer.subscription.updated` records `cancelAtPeriodEnd`; `customer.subscription.deleted` → `canceled`, access ends at `ended_at` and is never extended. `bi:sub-id:<subscription>` maps billing events back to the email. Subscribers sign in at `/?join` like firm members (token claim `k: "sub"`; `liveAccess` reads the record live and returns `plan`). Account page shows renew/cancel/payment status and a "Manage subscription" link. Stripe links (`DAY_PASS_URL`, `MONTHLY_PASS_URL`, `BILLING_PORTAL_URL`) live in `src/config/pricing.ts`; an empty one hides what depends on it.
- **Firm plan vs. own Monthly Pass**: `liveAccess` reads every plan a signed-in email could have and uses the best (active firm → active own pass → either in grace). A subscriber whose firm enrolls is moved onto a free seat on their next visit (`resolveSession` claims it; skipped if the firm is full or the owner removed them — `bi:user:<email>.removedFrom`, cleared by signing up again). When a firm plan ends, anyone still paying carries on with their own pass; others get a "continue on your own for $99/month" offer (checkout pre-filled with their email). Paying twice is flagged on the Account page and the usage page, and the daily cron emails the owner once per person per firm plan (`bi:overlap-alerted:*`); cancelling/refunding is done in Stripe. Usage page lists subscribers. Stripe webhook must subscribe to all five events above. Test: `node scripts/subscription-test.mjs`.
